...
int main()
{
	bn_t by, br1, br2, bm, bm_prime, r_m, r2_m, p1, p2, s, t;
	bn_clear(by);
	bn_clear(br1);
	bn_clear(br2);
	bn_clear(bm);
	...
	cp2bn(r2, br2);
	cp2bn(m, bm);
	cp2bn(m_prime, bm_prime);
	cp2bn(r_modm, r_m);
	cp2bn(r2_modm, r2_m);

	bn_mont_expmod(by, br1, bm, p1);
	bn_mont_expmod(p1, br2, bm, s);
	bn_print("s=0x",s);
	bn_mont_expmod(by, br2, bm, p2);
	bn_mont_expmod(p2, br1, bm, t);
	bn_print("t=0x", t);

	if (bn_cmp(s, t)) {
		printf("mont test FAILED\n");
		return -1;
	}
	else {
		printf("mont test PASSED\n");
		return 0;
	}
	/*
