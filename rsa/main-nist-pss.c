...
/* 186-3 RSA Test Vectors for PSS
 * This program has to be comipiled with CPPFLAGS=-DRSA_NIST_TEST
 * to enable rsassa_pss_encodepad() copy g_salt to salt 
 */ 

#ifdef RSA_NIST_TEST
extern uint8_t g_salt[512];
#else
uint8_t g_salt[512];
#endif
...
int sig_gen(char *filename)
{
	...
	while (!feof(fp)) {
		...
		while (fgets(buf, sizeof(buf), fp)) {
			...
			else if (strstr(buf, "SaltVal = ")) {value|=0x8;slen=hex2ba(buf+10, g_salt, sizeof(g_salt));}
			else if (strlen(buf) <= 2 && value == 0xF) break; /* \r\n */
		}
		if (value != 0xF) break; /* end of file */

		/* A bug in downloads/186-3rsatestvectors/SigVerPSS_*.rsp
		 * it use SaltValue = 00 to represent salt length = 0
		 * which is not correct.
		 */
		if ((slen==1) && !g_salt[0]) slen = 0;

		/* signature generation */
		nbits = bn_getmsbposn(prv.n);
		hash_id = str2hash_id(sha_name);

		rc |= rsassa_pss_init(&ctx, nbits-1, hash_id);
...
int sig_verify(char *filename)
{
	...
	while (!feof(fp)) {
		...
		while (fgets(buf, sizeof(buf), fp)) {
			...
			else if (strstr(buf, "SaltVal = ")) {value|=0x20;slen=hex2ba(buf+10, g_salt, sizeof(g_salt));}
			else if (strstr(buf, "Result = ")) {value|=0x40;result=buf[9]=='P'?0:-1;}
...
int main(int argc, char *argv[])
{
	...
	char sign_gen_filename[][128] = {
		"downloads/186-2rsatestvectors/SigGenPSS_186-2.txt",
		"downloads/186-3rsatestvectors/SigGenPSS_186-3.txt",
		"downloads/186-3rsatestvectors/SigGenPSS_186-3_TruncatedSHAs.txt",
	};
	char sign_ver_filename[][128] = {
		"downloads/186-2rsatestvectors/SigVerPSS_186-3.rsp",
		"downloads/186-3rsatestvectors/SigVerPSS_186-3.rsp",
		"downloads/186-3rsatestvectors/SigVerPSS_186-3_TruncatedSHAs.rsp",
	};
	...
	for (i=0; i<ARRAY_SIZE(sign_ver_filename); i++) {
		rc = sig_verify(sign_ver_filename[i]);
		if (rc) return rc;
	}
	return 0;
}
