...
/*
 * echo -n "value" | openssl dgst -sha224 -hmac "key"
 */
#include <assert.h>
#include <string.h>
#include <stdio.h>
...
int main(int argc, char *argv[])
{
	...
	uint8_t expected1[] = {"C7405E3AE058E8CD30B08B4140248581ED174CB34E1224BCC1EFC81B"};
	uint8_t keys2[] = {"000102030405060708090A0B0C0D0E0F101112131415161718191A1B"};
	uint8_t data2[] = {"Sample message for keylen<blocklen"};
	uint8_t expected2[] = {"E3D249A8CFB67EF8B7A169E9A0A599714A2CECBA65999A51BEB8FBBE"};
	...
	uint8_t expected3[] = {"91C52509E5AF8531601AE6230099D90BEF88AAEFB961F4080ABC014D"};
	...
	uint8_t expected4[] = {"D522F1DF596CA4B4B1C23D27BDE067D6153BA9725FD5CDE0AF4A2A42"};

	key_len = 64;
	hex2ba(keys1, key, key_len);
	hex2ba(expected1, expdgst, sizeof(dgst));
	hmac_init(&hmac, key, key_len, eHASH_SHA224);
