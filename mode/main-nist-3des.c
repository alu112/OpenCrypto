...
int main(int argc, char *argv[])
{
	...
	uint8_t key[192], iv[128],  pt[512], ct[512], buf[512];
	uint8_t Key[400], IV[256],  PT[1024], CT[1024];
	uint32_t value;
	...
	for (i=0; i<ARRAY_SIZE(filenames); i++) {
		...
		if (!strncmp(basename(filename), "TCBC", 4))
			init = cbc_init;
		else if (!strncmp(basename(filename), "TECB", 4))
			init = ecb_init;
		else if (!strncmp(basename(filename), "TCTR", 4))
			init = ctr_init;
		else if (!strncmp(basename(filename), "TOFB", 4))
			init = ofb_init;
		else if (!strncmp(basename(filename), "TCFB", 4))
			init = cfb_init;
		else {
		...
		while (!feof(fp)) {
			status = 0;
			value  = 0;
			if (!strncmp(basename(filename), "TECB", 4))
				value |= 0x02;
			while (fgets(buf, sizeof(buf), fp)) {
				line++;
				strtok(buf, "\r\n");
				if (token=strstr(buf, "# Key Length : ")) Keylen = atoi(token + 15);
				else if (strstr(buf, "KEYs = ")) {value|=0x01;strcpy(Key, buf+7);}
				else if (strstr(buf, "IV = "))  {value|=0x02;strcpy(IV,  buf+5);}
			...
			tdes_init(&tdes, key, Keylen*8);

			/* encrypt */
			memcpy(buf, pt, PTlen);
			init(&ctx, (blk_ctx_t *)&tdes, pad_algo, iv);
